...
export default function App() {
  ...
  async function addToCart(product) {
    try {
      ...
      if (e.response?.status === 401 || e.response?.status === 403) alert("Please login first.");
      console.error("Failed to add to cart", e);
    }
  }
  ...
  return auth ? (
    <div className="app-root">
      <Navbar setAuth={setAuth} cartCount={cartCount} onToggleSidebar={() => setSidebarOpen(v => !v)} sidebarOpen={sidebarOpen} />
      <div className={`layout container-fluid ${sidebarOpen ? "" : "sidebar-closed"}`}>
        <aside className={`sidebar-col ${sidebarOpen ? "open" : ""}`}>
          <Sidebar isAdmin={isAdmin} onNavigate={() => isMobile && setSidebarOpen(false)} />
        </aside>
        <main className="content-col">
          <div className="container py-4">
            <Routes>
              <Route path="/home" element={<Home addToCart={addToCart} isAdmin={isAdmin} />} />
              <Route path="/products" element={<Products addToCart={addToCart} isAdmin={isAdmin} />} />
              <Route path="/cart" element={<Cart isAuth={auth} clearCart={clearCart} />} />
              <Route path="/profile" element={<Profile />} />
              <Route path="*" element={<Navigate to="/home" />} />
            </Routes>
          </div>
      ...
      {isMobile && sidebarOpen && <div className="sidebar-backdrop" onClick={() => setSidebarOpen(false)}></div>}
      <Footer />
    </div>
  ) : (
    <Login setAuth={setAuth} />
